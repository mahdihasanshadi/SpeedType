...
| Postgres connection pool | `pg.Pool` in `lib/prisma.ts` set with `keepAlive: true`, `keepAliveInitialDelayMillis: 10_000`, `idleTimeoutMillis: 10_000` | Neon's pooler silently drops idle connections. `idleTimeoutMillis` alone did **not** fix this — 10s is pg's own default, so it was a no-op the first time; the connection still died under a real multi-minute idle gap during testing ("Connection terminated unexpectedly"). `keepAliveInitialDelayMillis` is what actually matters — it makes the OS send TCP keepalive probes soon after a connection goes idle instead of following the OS default (often minutes), which is what was letting Neon's proxy kill the socket out from under us. Verified fixed under realistic request timing after the change; always attach a `pool.on('error', ...)` listener too — an unhandled idle-client error can otherwise crash the process. |
| Hosting | Vercel | Native Next.js support, preview deploys per branch/PR |
| `pnpm test` occasional single-test timeout | Known flake, not a regression — re-run the specific file alone before treating it as a real failure | Vitest runs test files in parallel workers, each opening its own Neon connection; under that concurrent cold-start load `app/api/tests/summary/route.test.ts` has intermittently hit the 5s default timeout. Every occurrence so far has passed cleanly (3/3) when re-run in isolation (`pnpm exec vitest run app/api/tests/summary/route.test.ts`) — confirm that before investigating further. |
